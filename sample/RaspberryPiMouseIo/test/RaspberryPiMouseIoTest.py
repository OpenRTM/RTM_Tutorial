...
"""
 @file RaspberryPiMouseIoTest.py
...
raspberrypimouseiotest_spec = ["implementation_id", "RaspberryPiMouseIoTest",
		 "type_name",         "RaspberryPiMouseIoTest",
...
##
# @class RaspberryPiMouseIoTest
# @brief RaspberryPi Mouse Controller for Choreonoid Simulator
#
# Choreonoid用Raspberry Pi Mouse入出力コンポーネント
#
#
class RaspberryPiMouseIoTest(OpenRTM_aist.DataFlowComponentBase):
	...
	def __init__(self, manager):
		...
		距離センサのデータ
		 ...
		 - Number: 4
		"""
		self._ir_sensorIn = OpenRTM_aist.InPort("ir_sensor", self._d_ir_sensor)
	...
	def onInitialize(self):
		...
		# Set InPort buffers
		self.addInPort("ir_sensor",self._ir_sensorIn)
...
def RaspberryPiMouseIoTestInit(manager):
    profile = OpenRTM_aist.Properties(defaults_str=raspberrypimouseiotest_spec)
    manager.registerFactory(profile,
                            RaspberryPiMouseIoTest,
                            OpenRTM_aist.Delete)

def MyModuleInit(manager):
    RaspberryPiMouseIoTestInit(manager)

    # Create a component
    comp = manager.createComponent("RaspberryPiMouseIoTest")
