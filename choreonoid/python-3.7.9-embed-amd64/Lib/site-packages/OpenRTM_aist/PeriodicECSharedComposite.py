...
class PeriodicECOrganization(OpenRTM_aist.Organization_impl):
    ...
    def removeAllMembers(self):
        ...
        for member in self._rtcMembers:
            ...
            self.removeOrganizationFromTarget(member)
            self.startOwnedEC(member)
            OpenRTM_aist.Organization_impl.remove_member(
                self, member._profile.instance_name)
    ...
    def updateDelegatedPorts(self):
        ...
        self._rtcout.RTC_VERBOSE(
            "old    ports: %s",
            OpenRTM_aist.flatten(oldPorts))
        self._rtcout.RTC_VERBOSE(
            "new    ports: %s",
            OpenRTM_aist.flatten(newPorts))
        self._rtcout.RTC_VERBOSE(
            "remove ports: %s",
            OpenRTM_aist.flatten(removedPorts))
        self._rtcout.RTC_VERBOSE(
            "add    ports: %s",
            OpenRTM_aist.flatten(createdPorts))
...
class PeriodicECSharedComposite(OpenRTM_aist.DataFlowComponentBase):
    ...
    def __init__(self, manager):
        ...
        ).getLogbuf("rtobject.periodic_ec_shared")
    ...
    def onActivated(self, exec_handle):
        ...
        # since Python 2.5
        # self._rtcout.RTC_DEBUG("%d member RTC%s activated.", (len_,(lambda x:
        # if x > 1 else "was")(len_)))
...
def PeriodicECSharedCompositeInit(manager):
    profile = OpenRTM_aist.Properties(
        defaults_str=periodicecsharedcomposite_spec)
