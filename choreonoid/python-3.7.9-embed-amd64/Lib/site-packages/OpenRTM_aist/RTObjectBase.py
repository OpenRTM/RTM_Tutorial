...
class RTObjectBase:
    ...
    def __init__(self, manager=None, orb=None, poa=None):
        if manager:
            ...
            self._portAdmin = OpenRTM_aist.PortAdmin(
                self._manager.getORB(), self._manager.getPOA())
        else:
            self._manager = None
            self._orb = orb
            self._poa = poa
        ...
        self._configsets = OpenRTM_aist.ConfigAdmin(
            self._properties.getNode("conf"))
        self._profile = RTC.ComponentProfile(
            "", "", "", "", "", "", [], None, [])

        self._sdoservice = OpenRTM_aist.SdoServiceAdmin(self)
        self._SdoConfigImpl = OpenRTM_aist.Configuration_impl(
            self._configsets, self._sdoservice)
    ...
    def initialize(self):
        ...
        if self.createContexts(ec_args_) != RTC.RTC_OK:
            self._rtcout.RTC_ERROR(
                "EC creation failed. Maybe out of resources. Aborting.")
            return RTC.OUT_OF_RESOURCES

        # -- entering alive state --
        def toSTR_(x): return " was" if len(x) == 1 else "s were"
    ...
    def get_owned_contexts(self):
        ...
        OpenRTM_aist.CORBA_SeqUtil.for_each(
            self._ecMine, self.ec_copy(execlist))
        return execlist
    ...
    def get_participating_contexts(self):
        ...
        execlist = []
        OpenRTM_aist.CORBA_SeqUtil.for_each(
            self._ecOther, self.ec_copy(execlist))
        return execlist
    ...
    def get_component_profile(self):
        ...
        OpenRTM_aist.NVUtil.copyFromProperties(
            self._profile.properties, self._properties)
        return prop_
    ...
    def on_initialize(self):
        ...
        try:
            ...
            if ret != RTC.RTC_OK:
                self._rtcout.RTC_ERROR(
                    "onInitialize() returns an ERROR (%d)", ret._v)
            else:
        ...
        if self._configsets.haveConfig(active_set):
            self._rtcout.RTC_DEBUG(
                "Active configuration set: %s exists.", active_set)
            self._configsets.activateConfigurationSet(active_set)
            self._configsets.update(active_set)
            self._rtcout.RTC_INFO(
                "Initial active configuration set is %s.", active_set)
        else:
            self._rtcout.RTC_DEBUG(
                "Active configuration set: %s does not exists.", active_set)
    ...
    def get_service_profile(self, _id):
        ...
        try:
            index = OpenRTM_aist.CORBA_SeqUtil.find(
                self._sdoSvcProfiles, self.svc_name(_id))
    ...
    def get_configuration(self):
        ...
        if self._SdoConfig is None:
            raise SDOPackage.InterfaceNotImplemented(
    ...
    def get_status(self, name):
        ...
        index = OpenRTM_aist.CORBA_SeqUtil.find(
            self._sdoStatus, self.nv_name(name))
        if index < 0:
    ...
    def setInstanceName(self, instance_name):
        ...
        self._profile.instance_name = self._properties.getProperty(
            "instance_name")
    ...
    def getNamingNames(self):
        ...
        return [s.strip()
                for s in self._properties.getProperty("naming.names").split(",")]
    ...
    def addPort(self, port):
        ...
        if isinstance(port, OpenRTM_aist.CorbaPort):
            ...
            if prop:
                self._properties.getNode(propkey).mergeProperties(
                    self._properties.getNode("port.corba"))
    ...
    def addOutPort(self, name, outport):
        ...
        prop_ = copy.copy(self._properties.getNode(propkey))
        prop_.mergeProperties(
            self._properties.getNode("port.outport.dataport"))
    ...
    def removePort(self, port):
        ...
        if isinstance(port, OpenRTM_aist.PortBase) or isinstance(
                port, OpenRTM_aist.CorbaPort):
    ...
    def getPrivateContextOptions(self, ec_args):
        ...
        for ec_tmp in ecs_tmp_:
            ...
            if len(type_and_name_) == 2 and type_and_name_[1][-1] == ')':
                ...
                p_name_ = self._properties.findNode(
                    "ec." + p_.getProperty("name"))
