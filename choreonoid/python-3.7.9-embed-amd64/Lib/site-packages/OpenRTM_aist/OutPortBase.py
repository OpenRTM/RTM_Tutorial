...
class OutPortBase(OpenRTM_aist.PortBase, OpenRTM_aist.DataPortStatus):
    ...
    # @endif
    #
    # ReturnCode_t OutPortBase::subscribeInterfaces(const ConnectorProfile&
    # cprof)
    ...
    #
    # void
    # OutPortBase::unsubscribeInterfaces(const ConnectorProfile&
    # connector_profile)
    ...
    #
    # OutPortProvider*
    # OutPortBase::createProvider(ConnectorProfile& cprof, coil::Properties&
    # prop)
    ...
    def createProvider(self, cprof, prop):
        ...
        self._rtcout.RTC_DEBUG(
            "interface_type: %s",
            prop.getProperty("interface_type"))
        provider = OpenRTM_aist.OutPortProviderFactory.instance(
    ...
    def createConsumer(self, cprof, prop):
        ...
        self._rtcout.RTC_DEBUG(
            "interface_type: %s",
            prop.getProperty("interface_type"))
        consumer = OpenRTM_aist.InPortConsumerFactory.instance(
        ).createObject(prop.getProperty("interface_type"))
    ...
    def createConnector(self, cprof, prop, provider_=None, consumer_=None):
        ...
        try:
            ...
            if consumer_ is not None:
                connector = OpenRTM_aist.OutPortPushConnector(
                    profile, consumer_, self._listeners)
            elif provider_ is not None:
                connector = OpenRTM_aist.OutPortPullConnector(
    ...
    def createDuplexConnector(self, cprof, prop, provider_):
        ...
        connector = None
        try:
            connector = OpenRTM_aist.OutPortDuplexConnector(
                profile, provider_, self._listeners)
