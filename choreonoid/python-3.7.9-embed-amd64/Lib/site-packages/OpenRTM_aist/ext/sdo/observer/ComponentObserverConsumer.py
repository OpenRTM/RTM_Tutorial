...
class ComponentObserverConsumer(OpenRTM_aist.SdoServiceConsumerBase):
    ...
    def __init__(self):
        self._rtobj = None
        self._profile = None
        self._observer = OpenRTM_aist.CorbaConsumer(
            interfaceType=OpenRTM.ComponentObserver)
    ...
    def reinit(self, profile):
        if not self._observer._ptr()._is_equivalent(profile.service):
            tmp = OpenRTM_aist.CorbaConsumer(
                interfaceType=OpenRTM.ComponentObserver)
    ...
    def setListeners(self, prop):
        ...
        observed_ = [s.strip()
                     for s in prop.getProperty("observed_status").split(",")]
    ...
    def setDataPortInterval(self, prop):
        ...
        outportInterval = [0.0]
        if OpenRTM_aist.stringTo(outportInterval, prop.getProperty(
                "port_profile.send_event.min_interval")):
            self._outportInterval = OpenRTM_aist.TimeValue(outportInterval[0])

        inportInterval = [0.0]
        if OpenRTM_aist.stringTo(inportInterval, prop.getProperty(
                "port_profile.receive_event.min_interval")):
    ...
    def setHeartbeat(self, prop):
        ...
        if OpenRTM_aist.toBool(prop.getProperty(
            ...
            self._hbtaskid = OpenRTM_aist.Manager.instance().addTask(self.heartbeat, tm_)
    ...
    def unsetHeartbeat(self):
        if self._heartbeat:
            OpenRTM_aist.Manager.instance().removeTask(self._hbtaskid)
            self._heartbeat = False
        return
    ...
    class CompStatMsg:
        ...
        # void onGeneric(const char* msgprefix, UniqueId ec_id, ReturnCode_t
        # ret)
