...
class PortBase(RTC__POA.PortService):
    ...
    def connect(self, connector_profile):
        ...
        try:
            retval, connector_profile = connector_profile.ports[0].notify_connect(
                connector_profile)
            ...
            # return
            # connector_profile.ports[0].notify_connect(connector_profile)
    ...
    def notify_connect(self, connector_profile):
        ...
        default_value = OpenRTM_aist.toBool(
            self._properties.getProperty("allow_dup_connection"), "YES", "NO", False)

        if not OpenRTM_aist.toBool(prop.getProperty(
                "dataport.allow_dup_connection"), "YES", "NO", default_value):
        ...
        self.onSubscribeInterfaces(
            self.getName(), connector_profile, retval[2])
    ...
    def notify_disconnect(self, connector_id):
        ...
        OpenRTM_aist.CORBA_SeqUtil.erase(
            self._profile.connector_profiles, index)
    ...
    # @endif
    #
    # void setPortConnectListenerHolder(PortConnectListeners*
    # portconnListeners);
    ...
    def appendProperty(self, key, value):
        OpenRTM_aist.NVUtil.appendStringValue(
