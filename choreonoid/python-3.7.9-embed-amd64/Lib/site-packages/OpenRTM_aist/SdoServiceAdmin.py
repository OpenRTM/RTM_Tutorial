...
class SdoServiceAdmin:
    ...
    def init(self, rtobj):
        ...
        enabledProviderTypes = [s.strip() for s in prop.getProperty(
            "sdo.service.provider.enabled_services").split(",")]
    ...
    # SdoServiceAdmin::getServiceProviderProfiles()
        ...
        for consumerType in consumerTypes:
            if consumerType == str(sProfile.interface_type):
                ...
                self._rtcout.RTC_PARANOID(
                    "Available SDO serices in the factory: %s", str(
                        OpenRTM_aist.flatten(consumerTypes)))
                return True
