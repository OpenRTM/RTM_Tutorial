...
class CorbaNaming:
        ...
        for i in range(length):
            if i == length - 1:
                ...
                if self.objIsNamingContext(cxt):
                    cxt = self.bindOrResolveContext(
                        cxt, self.subName(name_list, i, i))
                else:
                    raise CosNaming.NamingContext.CannotProceed(
                        cxt, self.subName(name_list, i))
        return
    ...
    # @return
    # @endif
    # BindingList_var listByKind(const char* string_name,const char*
    # string_kind)
